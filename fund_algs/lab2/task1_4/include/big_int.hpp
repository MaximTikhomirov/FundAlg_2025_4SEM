...
inline std::ostream& operator<< (std::ostream& output, const BigInt& element){
    if (element.empty()){
        return output;
    }
    ...
    int len_base = (int) log10l(element.base());
...
inline std::istream& operator>>(std::istream& is, BigInt& bigInt) {
    std::string input;
    char c;
    is >> std::ws;
    c = is.peek();
